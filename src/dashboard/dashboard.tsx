...
import type { WorkerMessage, WorkerResult } from "./sum";
...
export default function Dashboard(): JSX.Element {
  ...
  useEffect(() => {
    const worker = new Worker(new URL("./sum.ts", import.meta.url), {
      type: "module",
    });
    console.log(worker);
  ...
  return (
    <div>
      ...
      {isCalculating && (
        <p>
          Вычисляю... Это может занять некоторое время, но UI не заблокирован!
        </p>
      )}
      {error && <p style={{ color: "red" }}>{error}</p>}
      {!isCalculating && sumResult !== null && (
        <p>
          Сумма от 0 до 1e8: <strong>{sumResult}</strong>
        </p>
      )}
